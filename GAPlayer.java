...
public class GAPlayer implements Player {
    ...
    public int evalMove(int[] move, int[][] board) {
        ...
        final int VWAL = 10; // weighting to block against a wall
        int WOPP = 0; 
        ...
        try {
            if (board[move[0] - 1][move[1] - 1] < 0) {
                WOPP = board[move[0] - 1][move[1] - 1];
                if (WOPP < 0){
                    value += VOPP * (WOPP * -1); // this is an opponents move, we like these
                } else value += VOPP;
        ...
        try {
            if (board[move[0] + 1][move[1] + 1] < 0) {
                WOPP = board[move[0] + 1][move[1] + 1] * -1;
                if (WOPP < 0){
                    value += VOPP * (WOPP * -1); // this is an opponents move, we like these
                } else value += VOPP;
            }
